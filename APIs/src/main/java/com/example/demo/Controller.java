...
import org.apache.catalina.User; import org.springframework.web.bind.annotation.GetMapping;import org.springframework.web.bind.annotation.PathVariable;import org.springframework.web.bind.annotation.PostMapping;import org.springframework.web.bind.annotation.RequestBody;import org.springframework.web.bind.annotation.RestController;import com.example.demo.Response;import com.fasterxml.jackson.databind.node.ObjectNode;import Client.database;import Client.login;import Client.register;import Client.wallet;import admin.admin;import admin.ReqARefund;import companies.AccCredentials;import companies.VerifiedCompanies;import companies.etisalat;import companies.orange;import companies.vodafone;import companies.we;import paying.Iservice;import paying.serviceFactory;import paying.transaction;
...
public class Controller {
	...
	VerifiedCompanies ets;
	VerifiedCompanies voda ;
	VerifiedCompanies orange;
	VerifiedCompanies we;
	...
	Controller() {
		 AccCredentials etsacc = new AccCredentials("011ac", 14520);
		 ets = new etisalat("011", etsacc);

		data.addCompany(ets);
		data.addAcc(etsacc, ets);

		AccCredentials vodacc = new AccCredentials("010ac", 1424);
		voda = new vodafone("010", vodacc);

		data.addCompany(voda);
		data.addAcc(vodacc, voda);

		AccCredentials orangeacc = new AccCredentials("012ac", 123457);
		orange = new orange("012", orangeacc);
		
		data.addCompany(orange);
		data.addAcc(orangeacc, orange);

		AccCredentials weacc = new AccCredentials("015ac", 5895);
	    we = new we("015", weacc);
	...
	public Response Login(@RequestBody ObjectNode objectNode ) {
		...
		String pass = objectNode.get("pass").asText();
		Response response = new Response();
		int res = loging.LoginAUser(em, pass, data);
		if (res == 2) {
            response.setStatus(false);
	...
	public Response ServiceCompany(@PathVariable("s")char s,@PathVariable("c")String c,@RequestBody ObjectNode objectNode) {
	 ...
	 public Response RefundRequest(@RequestBody ObjectNode objectNode) {
		 ...
		 if((loging.getCurrentuser().getEmail()).equals((t.getUser().getEmail()))) {
			 ReqARefund rr=new ReqARefund(loging.getCurrentuser().getEmail(),t);
	...
	public Response AdminAccept(@PathVariable("ReqId")String ReqId) {
	 ...
	 public static void paying(double fee,Iservice service, VerifiedCompanies c, login l, database data) {
			transaction t;
